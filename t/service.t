...
sub check_service_buttons {
	...
	$is_ok &= ok($driver->find_child_element($lp, 'btn_own_services'),				"found button:\t'Eigene'");
	$is_ok &= ok($driver->find_child_element($lp, 'btn_used_services'),				"found button:\t'Genutzte'");
	$is_ok &= ok($driver->find_child_element($lp, 'btn_usable_services'),			"found button:\t'Nutzbare'");
	$is_ok &= ok($driver->find_child_element($lp, 'btn_search_services'),			"found button:\t'Suche'");
	$is_ok &= ok($driver->find_child_element($lp, 'btn_services_print_all'), 	"found button:\tprint all");
	$is_ok &= ok($driver->find_child_element($lp, 'btn_services_print'), 			"found button:\tprint");

	# check buttons on right panel
	$is_ok &= ok($driver->find_child_element($rp, 'btn_service_details'), 	"found button:\t'Details zum Dienst'");
	$is_ok &= ok($driver->find_child_element($rp, 'btn_service_user'), 			"found button:\t'Benutzer (User) des Dienstes'");
	$is_ok &= ok($driver->find_child_element($rp, 'btn_print_rules',), 			"found button:\tprint rules");
	$is_ok &= ok($driver->find_child_element($rp, 'btn_service_user_add'), 	"found button:\tadd user to service");
	$is_ok &= ok($driver->find_child_element($rp, 'btn_service_user_del'), 	"found button:\tdelete user from service");
...
sub check_own_services_grid {
	...
	my @grid = $driver->find_child_elements($lp, './/*[contains(@class, "x-grid-row")]', 'xpath');
...
#	for (my $i = 0; $i < @grid; $i++) { print "$i: ". $grid[$i]->get_attribute('id'). ", " . $grid[$i]->get_text . "\n"; }
	...
	ok($grid_head_text =~ 'Dienstname\s\(Anzahl:\s\d+\)', "found header:\town services grid");
	ok($grid_head_text =~ /([0-9]+)/ ? ($1 eq scalar @grid) : 0, "header contains number of elements");

	my @contains = ("Test1", "Test10", "Test11", "Test2",
									"Test3", "Test3a", "Test4",  "Test5",
									"Test6", "Test7",  "Test8",  "Test9"
								);
	if (!ok($driver->grid_contains(\$lp, \1, \0, \@contains), "grid contains all tests services")){	
	...
	$driver->click;
	@grid	= $driver->find_child_elements($lp, './/*[contains(@class, "x-grid-row")]', 'xpath');
	$is_ok &= $driver->is_grid_in_order(\@grid, \1, \1, \1, \0);
	if (!ok($is_ok, "grid changes correctly")) { die("own service grid"); }
...
sub test_print_services{
  ...
  my $same_header = $service_head eq $driver->find_child_element($winprin, '//th', 'xpath')->get_text;

  ok($same_header	&& $driver->comp_array(\@service_cells, \@print_cells), "print services contains all information");
...
sub service_details{
	...
	ok($driver->find_child_element(	$rp, 'cb_expand_users'), 	"found checkbox:\texpand user");
	ok($driver->find_child_element(	$rp, 'cb_show_names'), 		"found checkbox:\tshow names");
	ok($driver->find_child_element(	$rp, 'cb_filter_search'), "found checkbox:\tfilter search");
	...
	my @regex = ('permit', 'User', '(2(0..5)(0..5)|1\d\d|\d\d|\d)\.(2(0..5)(0..5)|1\d\d|\d\d|\d)\.(2(0..5)(0..5)|1\d\d|\d\d|\d)\.(2(0..5)(0..5)|1\d\d|\d\d|\d)', '(udp|tcp)\s\d+');
	$is_ok = 1;
	$is_ok = $driver->check_sytax_grid(\@gcc, \5, \0, \@regex);
	
	$driver->find_child_element($rp, 'cb_expand_users')->click;	
	#regex matches anschauen
	$regex[1] = '((2(0..5)(0..5)|1\d\d|\d\d|\d)\.(2(0..5)(0..5)|1\d\d|\d\d|\d)\.(2(0..5)(0..5)|1\d\d|\d\d|\d)\.(2(0..5)(0..5)|1\d\d|\d\d|\d)(\-\/(2(0..5)(0..5)|1\d\d|\d\d|\d)\.(2(0..5)(0..5)|1\d\d|\d\d|\d)\.(2(0..5)(0..5)|1\d\d|\d\d|\d)\.(2(0..5)(0..5)|1\d\d|\d\d|\d))?)';
	...
	# Verantwortliche
	ok($driver->find_child_element($rp, 'ownerEmails')->get_text =~ 'Verantwortliche\sf.r\sz\sguest', "found panel:\tresponsible email contact");
}
...
# -------------
	...
	if (!ok($search_window, "found search 'window'")){die("no search window -> no search");}
	...
	ok($driver->find_child_element($search_window, 'cb_search_limited'				), "found checkbox:\tsearch temporary services");
	...
	if (!ok($search_window->is_displayed, "keep search in foreground")){ die("search window vanished:\n$@"); }
	...
	# close popup
	$driver->find_child_element($driver->find_element('x-message-box', 'class'), 'x-btn-button', 'class')
