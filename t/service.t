...
sub check_own_services_grid {
    ...
    my @grid = $driver->find_child_elements( $lp,
...
#	for (my $i = 0; $i < @grid; $i++) { print "$i: ". $grid[$i]->get_attribute('id'). ", " . $grid[$i]->get_text . "\n"; }
    ...
    if (!ok($driver->grid_contains( \$lp, \1, \0, \@contains ),
    ...
    $driver->click;
    @grid = $driver->find_child_elements( $lp,
        './/*[contains(@class, "x-grid-row")]', 'xpath' );
    $is_ok &= $driver->is_grid_in_order( \@grid, \1, \1, \1, \0 );
...
sub test_print_services {
    ...
    my $same_header = $service_head eq
...
#for (my $i = 0; $i < @pseudo_input; $i++) { print "$i: ". $pseudo_input[$i]->get_attribute('id'). ", " . $pseudo_input[$i]->get_value . "\n"; }
    ...
    ok( $pseudo_input[2]->get_value eq 'z',
...
sub search_tab {
    ...
    $driver->find_child_element( $search_window, 'cb_search_keep_foreground' )
        ->click;
    ...
    $driver->find_child_element( $search_window, 'cb_search_case_sensitive' )
        ->click;
    $driver->find_child_element( $search_window, 'btn_search_start' )->click;
