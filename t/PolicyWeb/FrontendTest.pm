...
sub grid_contains {
	...
	my @grid_cells = $driver -> find_child_elements(
		$grid_parent, 'x-grid-cell', 'class');
...
sub is_order_after_change {
	...
	$driver->is_grid_in_order(\@grid_cells, \$row, \$row, \-1, \$column) || (return 0);
	...
	for (my $i = $column + 1; $i < $row; $i++) {
		...
		@grid_cells = $driver->find_child_elements($grid, 'x-grid-cell', 'class');
		$driver->is_grid_in_order(\@grid_cells, \$row, \$row, \-1, \$i) || (return 0);
...
sub comp_array{
	...
	for (my $i = 0; $i < @a; $i++) { if ($a[$i] ne $b[$i]->get_text) {return 0;} }
	return 1;
}
