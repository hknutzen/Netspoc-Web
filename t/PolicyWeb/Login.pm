...
sub find_top_buttons {
    ...
    subtest "top elements" => sub {
        ...
        ok(wait_until { $driver->find_element('//div[text()="Stand"]', 'xpath') },
            "found text:\t'Stand'");
        ...
        ok(
            wait_until {
                $driver->find_element('//div[text()="Verantwortungsbereich"]', 'xpath')
            },
            "found text:\t'Verantwortungsbereich'"
          );
        ok(wait_until { $driver->find_element('list_owner') }, "found combo:\towner");

        ok(wait_until { $driver->find_element('//div[text()="Abmelden"]', 'xpath') },
            "found text:\t'Abmelden'");

        ok(wait_until { $driver->find_element('btn_logout') }, "found button:\tlogout");

        ok(wait_until { $driver->find_element('btn_info') }, "found button:\tinfo");
    };
}
